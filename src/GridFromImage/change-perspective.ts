/**
 * Source https://github.com/Volst/change-perspective
 *
 * Flipped the algebra of the applyTransform to receive the destination coordinates
 */
...
export function applyTransform(coeffs: QuadPoints, x: number, y: number) {
    const [a, b, c, d, e, f, g, h] = coeffs;

    const v4u = (f - y) / (h * y - e + g * y - d);
    const u = (x * h * v4u + x - b * v4u - c) / (a - x * g);

    const u4v = (c - x) / (g * x - a + h * x - b);
    const v = (y * g * u4v + y - d * u4v - f) / (e - y * h);

    return [u, v] as SinglePoint;
}
